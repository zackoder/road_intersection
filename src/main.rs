...
use car::Car;
use traffic_light::{TrafficLight, LightState, Direction};
...
async fn main() {
    ...
    let mut lights: Vec<TrafficLight> = vec![
        TrafficLight::new(vec2(center_x - lane_space - 50.0, center_y - 50.0 - 60.0), Direction::North),
        ...
        TrafficLight::new(vec2(center_x - lane_space - 50.0, center_y + lane_space), Direction::East),
    ];
    ...
    loop {
        ...
        if click >= 0.4 {
            if is_key_pressed(KeyCode::Up) && !is_lane_full(&cars, Direction::South, vec2(center_x, height_screen)) {
                click = 0.0;
                cars.push(Car::new(Direction::South, vec2(center_x, height_screen)));
            }
            if is_key_pressed(KeyCode::Down) && !is_lane_full(&cars, Direction::North, vec2(center_x - 30.0, 0.0)) {
                click = 0.0;
                cars.push(Car::new(Direction::North, vec2(center_x - 30.0, 0.0)));
            }
            if is_key_pressed(KeyCode::Left) && !is_lane_full(&cars, Direction::East, vec2(0.0, center_y)) {
                click = 0.0;
                cars.push(Car::new(Direction::East, vec2(0.0, center_y)));
            }
            if is_key_pressed(KeyCode::Right) && !is_lane_full(&cars, Direction::West, vec2(width_screen, center_y - 30.0)) {
                click = 0.0;
                cars.push(Car::new(Direction::West, vec2(width_screen, center_y - 30.0)));
            }
        }
        ...
        cars.retain(|car| {
            car.position.x > -50.0 && car.position.x < width_screen + 50.0 &&
            car.position.y > -50.0 && car.position.y < height_screen + 50.0
        });
        ...
        let other_cars_data: Vec<(Vec2, Direction)> =
            cars.iter().map(|car| (car.position, car.start_direction)).collect();

        for (idx, car) in cars.iter_mut().enumerate() {
            let other_cars: Vec<(Vec2, Direction)> = other_cars_data.iter()
                .enumerate()
                .filter(|(i, _)| *i != idx)
                .map(|(_, &data)| data)
                .collect();
